    ...
    def _generate_host_update(self):
        ...
        if self._node_sensor.total_memory["percent"] >= self._host_memory_usage_threshold \
           and not self.high_usage['memory']:
            ...
            if self.usage_time_map['memory'] - previous_check_time >= self._high_memory_usage_wait_threshold:
                ...
                fault_event = "Host memory usage has increased to {}%, beyond the configured threshold of {}% for more than {} seconds.".format(
                    self._node_sensor.total_memory["percent"], self._host_memory_usage_threshold, self._high_memory_usage_wait_threshold)
        ...
        if self._node_sensor.total_memory["percent"] < self._host_memory_usage_threshold:
            if not self.high_usage['memory']:
                ...
                # Create the memory data message and hand it over to the egress processor to transmit
                fault_resolved_event = "Host memory usage has decreased to {}%, lesser than the configured threshold of {}%.".format(
    ...
    def _generate_cpu_data(self):
        ...
        if self._node_sensor.cpu_usage >= self._cpu_usage_threshold \
           and not self.high_usage['cpu']:
            ...
            if self.usage_time_map['cpu'] - previous_check_time >= self._high_cpu_usage_wait_threshold:
                ...
                fault_event = "CPU usage has increased to {}%, beyond the configured threshold of {}% for more than {} seconds.".format(
                    self._node_sensor.cpu_usage, self._cpu_usage_threshold, self._high_cpu_usage_wait_threshold)
                logger.warning(fault_event)
        ...
        if self._node_sensor.cpu_usage < self._cpu_usage_threshold:
            if not self.high_usage['cpu']:
                ...
                # Create the cpu usage data message and hand it over to the egress processor to transmit
                fault_resolved_event = "CPU usage has decreased to {}%, lesser than the configured threshold of {}%.".format(
    ...
    def _generate_disk_space_alert(self):
        ...
        if self._node_sensor.disk_used_percentage >= self._disk_usage_threshold \
           and not self.high_usage['disk']:
            ...
            fault_event = "Disk usage has increased to {}%, beyond the configured threshold of {}%.".format(
        ...
        if self._node_sensor.disk_used_percentage <= self._disk_usage_threshold \
           and self.high_usage['disk']:
            ...
            # Create the disk space data message and hand it over to the egress processor to transmit
            fault_resolved_event = "Disk usage has decreased to {}%, lesser than the configured threshold of {}%.".format(
                self._node_sensor.disk_used_percentage, self._disk_usage_threshold)
            logger.warning(fault_resolved_event)
            diskSpaceAlertMsg = DiskSpaceAlertMsg(self._node_sensor.host_id,
                                    self._epoch_time,
