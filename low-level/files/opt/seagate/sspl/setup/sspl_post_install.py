...
class SSPLPostInstall:
    ...
    def install_sspl_service_files(self):
        ...
        currentProduct = "%s.%s" % (consts.SSPL_SERVICE_FILE, self.product)
        if (self.product == "SINGLE" and not os.path.exists(currentProduct)) or \
                (self.product == "DUAL" and not os.path.exists(currentProduct)):
            os.symlink("%s.%s" % (consts.SSPL_SERVICE_FILE, self.product),
                       currentProduct)
        if self.product == "CLUSTER" and not os.path.exists(currentProduct):
            os.symlink("%s.LR2" % consts.SSPL_SERVICE_FILE, currentProduct)
        shutil.copyfile(currentProduct,
                        "/etc/systemd/system/%s" % consts.SERVICE_NAME)

    def enable_sspl_service(self):
        """Enable sspl service."""
        self.dbus_service.enable(consts.SERVICE_NAME)
