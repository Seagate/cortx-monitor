...
if [[ "$kernel_version" != "$req_kernel_version" ]]; then
    echo "ERROR: Kernel version is wrong. Required: $req_kernel_version installed: $kernel_version"
    exit 1
else
    echo "Done."
fi
...
if grep -E -q "Red Hat.*7.7" /etc/*-release; then
    ...
    if [[ "$disable_sub_mgr_opt" == true ]]; then
        echo "\nINFO: Ignoring --disable-sub-mgr since it's not applicable for CentOS.."
    fi
    echo "INFO: Creating repos for Cotrx"
...
echo -n "INFO: Disabling default time syncronization mechanism..........."
