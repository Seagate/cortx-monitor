...
class RealStorDiskSensor(SensorThread, InternalMsgQ):
    ...
    def _rss_raise_disk_alert(self, alert_type, disk_info):
        ...
        disk[encl] = self.rssencl.LDR_R2_ENCL
    ...
    def rss_cliapi_poll_disks(self, disk):
        ...
        if not response:
            logger.warn(f"{self.rssencl.LDR_R2_ENCL}:: Disks status unavailable as ws request {url} failed")
            return

        if response.status_code != self.rssencl.ws.HTTP_OK:
            if url.find(self.rssencl.ws.LOOPBACK) == -1:
                logger.error(f"{self.rssencl.LDR_R2_ENCL}:: http request {url} to poll disks failed with \
