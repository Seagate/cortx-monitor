...
class RAIDIntegritySensor(SensorThread, InternalMsgQ):
    ...
    def _update_raid_device_file(self, device):
        try:
            ...
            sync_action_file = RaidDataConfig.SYNC_ACTION_FILE.value
            while raid_check <= RaidDataConfig.MAX_RETRIES.value:
                CHECK_COMMAND = "echo 'check' > " + raid_dir + device + sync_action_file
                ...
                if error:
                    logger.warn("Failed in executing command:{} {}."
                                .format(CHECK_COMMAND, error))
                    raid_check += 1
                    time.sleep(1)
                else:
                    logger.debug("RAID device state is changed to 'check'")
                    status = "success"
                    break
            return status
        except Exception as ae:
    ...
    def _retry_execution(self, function, device):
        while True:
            logger.debug("Executing function:{} after {} time interval"
                         .format(function, RaidDataConfig.NEXT_ITERATION_TIME.value))
