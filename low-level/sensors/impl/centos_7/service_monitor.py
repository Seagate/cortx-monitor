...
class ActiveState:
    ...
    @staticmethod
    def transit(service):
        inactive_services.discard(service.service)
...
class InactiveState:
    ...
    @staticmethod
    def transit(service):
        inactive_services.add(service.service)
...
class DisabledState:
    ...
    def transit(service):
        inactive_services.discard(service.service)
        properties_changed_not_connected_services.discard(service.service)
        service.state = "N/A"
        if service.signal:
...
class Service:
    ...
    def dump(self):
        ...
        store.put(data, f"{DUMP_PATH}/{self.service}")
...
class ServiceMonitor(SensorThread, InternalMsgQ):
    ...
    def run(self):
        ...
        try:
            ...
            iterations_to_check_for_inactive_services = int(
                self.polling_frequency / self.thread_sleep)
            while self.is_running():
                # At interval of 'thread_sleep' check for events occured for
                # registered services and process them
                ...
                if not iterations % iterations_to_check_for_inactive_services:
                    ...
                    # Initialize errored(serivice_to_monitor - intialized service) service again
    ...
    def initialize_service(self, service_name):
        try:
            ...
            logger.error(f"Failed to initialize service {service_name},"
                         f"initialization will be retired in"
                         f"{self.polling_frequency} seconds")
    ...
    def connect_to_unit_file_changed_handler(self):
        self._manager.connect_to_signal('UnitFilesChanged',
                                        self.unit_state_changed_handler,
                                        dbus_interface=MANAGER_IFACE)
    ...
    def unit_state_changed_handler(self):
    ...
    def connect_to_prop_changed_signal(self, service):
        ...
        try:
            ...
            if service.service not in properties_changed_not_connected_services:
                self.raise_alert(service, FailedAlert)
                properties_changed_not_connected_services.add(service.service)
        else:
            properties_changed_not_connected_services.discard(service.service)
            ...
            "sensor_request_type": {
                "service_status_alert": {
                    ...
                    "info": {
                        ...
                        "impact": alert["impact"].value.format(service.service),
