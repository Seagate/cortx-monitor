...
class DiskMonitor(SensorThread, InternalMsgQ):
    """Sensor to monitor drive add/remove events and
       to perform SMART on drives
    """
    ...
    def _is_smart_supported(self):
        ...
        result, _, _ = SimpleProcess("sudo facter is_virtual").run()
        if result and 'true' in bytes.decode(result):
    ...
    def _schedule_SMART_test(self, drive_path, test_type ="short", serial_number =None):
        ...
        # A request was received to run a SMART test on a SAS drive (RAID drives)
            ...
            response, error, _ = SimpleProcess(command).run()
            ...
            if len(error) > 0:
                ...
                if "SMART Health Status: OK" not in bytes.decode(response):
