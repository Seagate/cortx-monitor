...
Scenario: Change 12 drives to inuse_failed status and examine all responses
	Given that all drives are set to "inuse_ok" and sspl is started
	When I set "12" drives to "inuse_failed"
	Then SSPL_LL transmits JSON msgs with status "inuse_failed" for "12" drives for enclosure "SHU0951732G1GXC"

Scenario: Change 12 drives to inuse_ok status and examine all responses
	Given that all drives are set to "inuse_failed" and sspl is started
	When I set "12" drives to "inuse_ok"
	Then SSPL_LL transmits JSON msgs with status "inuse_ok" for "12" drives for enclosure "SHU0951732G1GXC"
