...
#TODO CORTEX_RENAME rename eos-py-utils to cortx-py-utils
...
if [ "$TEST" == true ]
then
    tar -czvf ${RPM_BUILD_PATH}/SOURCES/$PRODUCT-sspl-test-${VERSION}.tgz -C ${DIST} sspl_test
fi

if [ "$CLI" == true ]
then
tar -czvf ${RPM_BUILD_PATH}/SOURCES/$PRODUCT-sspl-cli-${VERSION}.tgz -C ${DIST} cli
fi

tar -czvf ${RPM_BUILD_PATH}/SOURCES/$PRODUCT-sspl-${VERSION}.tgz -C ${DIST} sspl
...
rpmbuild --define "version $VERSION" --define "git_rev $GIT_VER" \
    --define "_topdir $TOPDIR" --define "product $PRODUCT" -bb $TMPDIR/sspl-ll.spec
rpmbuild --define "version $VERSION" --define "git_rev $GIT_VER" \
    --define "_topdir $TOPDIR" --define "product $PRODUCT" -bb $TMPDIR/libsspl_sec.spec

if [ "$CLI" == true ]
then
rpmbuild --define "version $VERSION" --define "git_rev $GIT_VER" \
    --define "_topdir $TOPDIR" --define "product $PRODUCT" -bb $TMPDIR/sspl_cli.spec
fi

if [ "$TEST" == true ]
then
    rpmbuild --define "version $VERSION" --define "git_rev $GIT_VER" \
        --define "_topdir $TOPDIR" --define "product $PRODUCT" -bb $TMPDIR/sspl-test.spec
fi
