...
sleep 5s
timestamp=$(stat /var/cortx/sspl/sspl_xml_coverage_report.xml | grep Modify | cut -d ' ' -f2,3)
echo "${timestamp} : The Code Coverage report is saved at /var/cortx/sspl/sspl_xml_coverage_report.xml"
