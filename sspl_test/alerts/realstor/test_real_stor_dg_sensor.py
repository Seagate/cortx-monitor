...
def test_real_stor_dg_sensor(agrs):
    ...
    while not world.sspl_modules[IngressProcessorTests.name()]._is_my_msgQ_empty():
        ...
        try:
            ...
            time.sleep(0.1)
            if msg_type['info']['resource_type'] == "enclosure:cortx:disk_group":
                dg_sensor_msg = msg_type
                break
    ...
    assert(dg_sensor_specific_info.get("cvg_name") is not None)
    assert(dg_sensor_specific_info.get("cvg_id") is not None)
